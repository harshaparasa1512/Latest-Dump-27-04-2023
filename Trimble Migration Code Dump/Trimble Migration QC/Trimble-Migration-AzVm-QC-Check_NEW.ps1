...
Function Get-DC-QC {
    ...
    $serveruptime = Invoke-AzVMRunCommand -ResourceGroupName $AzResourceGroupName -VMName $AzVM.Name -CommandId 'RunPowerShellScript' `
    -ScriptString "net statistics workstation" -Verbose
    
    $INSTALLED = $serveruptime.Value[0].Message  #Checking the Output message

    $Folder = 'Available-Server-uptime'

    if (Test-Path -Path $Folder) {
        Write-Host "Available-Server-uptime Folder exists!"
    }
    else {
        New-Item -Path 'Available-Server-uptime' -ItemType Directory -Force
    }




    $INSTALLED | Out-File -FilePath "Available-Server-uptime\$($AzVM.Name)-Available-Server-uptime-$((Get-Date).ToUniversalTime().ToString('yyyyMMddTHHmmss')).txt"


###################################################validate boot diagnostics#######################################
$bootdiagnostics = Get-AzVMBootDiagnosticsData -ResourceGroupName $AzResourceGroupName -VMName $AzVM.Name -Windows -LocalPath "C:\Users\hparasa\Pictures\Final Download\Post config qc\validate-boot-diagnostics"
...
#$INSTALLED = $bootdiagnostics.Value[0].Message  #Checking the Output message

$Folder = 'validate-boot-diagnostics'

if (Test-Path -Path $Folder) {

Write-Host "validate-boot-diagnostics Folder exists!"

}

else {

New-Item -Path 'validate-boot-diagnostics' -ItemType Directory -Force

}
#################################################LOG-IN-USERS################################################################
$users = Invoke-AzVMRunCommand -ResourceGroupName $AzResourceGroupName -VMName $AzVM.Name -CommandId 'RunPowerShellScript' `
-ScriptString "query user" -Verbose


$INSTALLED = $users.Value[0].Message  #Checking the Output message
$Folder = 'Log-in-Users'
if (Test-Path -Path $Folder) {
Write-Host "Log-in-Users Folder exists!"
}
else {
New-Item -Path 'Log-in-Users' -ItemType Directory -Force
}

$INSTALLED | Out-File -FilePath "Log-in-Users\$($AzVM.Name)-Log-in-Users-$((Get-Date).ToUniversalTime().ToString(‘yyyyMMddTHHmmss’)).txt"
#######################################################################################################
...
############################################################################
    ...
    If ($INSTALLED.Contains("TeamViewer")) {
        $VMCheckData.Add("UNISTALL_TeamViewer", "Still Available")
    }
    else {
        $VMCheckData.Add("UNISTALL_TeamViewer", "Uninstalled")
    }
    ...
    if ($AzVM.Name -match "SQL") {
        ...
        # SQL: Register Azure SQL VMs Check

        $Sqlvm = Get-AzSqlVM -ResourceGroupName $AzResourceGroupName -Name $azvm.Name

        if ($Sqlvm) {            
            $VMCheckData.Add("Azure_SQL_VMs_Check", $Sqlvm.Name)
        }
        else {
            $VMCheckData.Add("Azure_SQL_VMs_Check", "Unavailable")
        }

        # SQL: Enable Backup for SQL VM Checking

        try {
            $azvmsqlextension = Get-AzVMSqlServerExtension -VMName $AzVM.Name -ResourceGroupName $AzResourceGroupName -ErrorAction Ignore
            $storageuri = $azvmsqlextension.AutoBackupSettings.StorageUrl
            
            if ($storageuri -ne 'NOTSET') {
                $VMCheckData.Add("Azure_SQL_VMs_Backup_Check", $storageuri)
            }
            else {
                $VMCheckData.Add("Azure_SQL_VMs_Backup_Check", "No Backup")
            }
        }
        catch {
            $VMCheckData.Add("Azure_SQL_VMs_Backup_Check", "No Backup")
