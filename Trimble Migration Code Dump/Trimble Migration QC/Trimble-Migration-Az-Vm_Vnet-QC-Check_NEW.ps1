##############################################################################
# Script Name   : Trimble-Migration-Az-Vm_Vnet-QC-Check.ps1
# Author        : Naresh Vanamala
# Created Date  : 26th Dec 2022
# Version       : 1.0
# Description   : PowerShell Script is useful for performing the Trimble    
#                  Migration QC Check for Server and Network Build Configuration
# Test Example  : 
##############################################################################
...
$TMAzQC = Get-Content -Path Trimble-Migration-Az-Vm_Vnet-QC-Check.json | ConvertFrom-Json
...
$ResourceType = $TMAzQC.ResourceType
$SendEmail = $TMAzQC.SendEmail
...
Function Get-VM-Data {
    ...
    $DataFromAzure.Add('OSDiskcaching', $VM.StorageProfile.OsDisk.Caching)
    $DataFromAzure.Add('DataDiskInGB', $VM.StorageProfile.dataDisks.diskSizeGB)
    $DataFromAzure.Add('DataDiskcaching', $VM.StorageProfile.DataDisks[0].caching)
    $DataFromAzure.Add('DataDiskcaching1', $VM.StorageProfile.DataDisks[1].caching)
    $DataFromAzure.Add('DataDiskcaching2', $VM.StorageProfile.DataDisks[2].caching)
    $DataFromAzure.Add('DataDiskcaching3', $VM.StorageProfile.DataDisks[3].caching)
...
Function ValidateValues() {
    ...
    FOREACH ($Item in $ExpectedValues.GetEnumerator()) {
        ...
        ELSEIF ($($Item.ExpectedValue) -eq 'NA' -or $($Item.ExpectedValue) -eq '') {
            ...
            $Obj | Add-Member -Name Result -MemberType NoteProperty -Value "$(if ($($Item.ExpectedValue) -eq $DataFromAzure.Get_Item($($Item.Parameter))) { '' } else { '' })";    
            $Obj | Add-Member -Name PerformedAt -MemberType NoteProperty -Value $([System.DateTime]::UtcNow)
            $Result += $Obj
        }
    }
...
Get-ChildItem $InputFolderPath -Filter '*.csv' -Name | ForEach-Object { 
    ...
    $removetempvms = Get-AzResource -ResourceGroupName HRT_Heritage_Transport |Select Name |Where-Object{$_.Name -like "*temp*"}

    $INSTALLED = $removetempvms  #Checking the Output message
    $Folder = 'Remove-Temp-List'
    if (Test-Path -Path $Folder) {
        Write-Host "Remove-Temp-List Folder exists!"
    }
    else {
        New-Item -Path 'Remove-Temp-List ' -ItemType Directory -Force
    }
    
    $INSTALLED | Out-File -FilePath "Remove-Temp-List\$($AzVM.Name)-Remove-tempvm-$((Get-Date).ToUniversalTime().ToString(‘yyyyMMddTHHmmss’)).txt"
